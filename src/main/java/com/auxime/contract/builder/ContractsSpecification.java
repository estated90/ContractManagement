...
public class ContractsSpecification {
        ...
        public Specification<Cape> getAllCape(String filter, LocalDate startDate, LocalDate endDate,
                ...
                return (root, query, criteriaBuilder) -> {
                        ...
                        List<Predicate> predicates = commonfields(root, criteriaBuilder,
        ...
        public List<Predicate> commonfields(Root<?> root,
                ...
                if (startDate != null && endDate != null) {
                        predicates.add(criteriaBuilder.between(root.get(STARTING_DATE), startDate, endDate));
                }
                if (startDate != null) {
                        predicates.add(criteriaBuilder.greaterThanOrEqualTo(root.get(STARTING_DATE), startDate));
                }
                if (endDate != null) {
                        predicates.add(criteriaBuilder.lessThanOrEqualTo(root.get(STARTING_DATE), endDate));
                }
                predicates.add(criteriaBuilder.equal(root.get("status"), true));
                return predicates;
        }

        public Specification<Contract> filterSqlContracts(String filter, LocalDate startDate, LocalDate endDate,
                ContractState contractState, PortageCompanies structureContract) {
                        return (root, query, criteriaBuilder) -> {
                                Map<String, LocalDate> dates = new HashMap<>();
                                dates.put(START_DATE, startDate);
                                ...
                                List<Predicate> predicates = commonfields(root, criteriaBuilder,
                                                filter, dates, contractState, structureContract);
                                query.orderBy(criteriaBuilder.asc(root.get(STARTING_DATE)));
                                query.orderBy(criteriaBuilder.asc(root.get("contractTypology")));
                                query.distinct(true);
                                return criteriaBuilder.and(predicates.toArray(new Predicate[0]));
                        };
        }
